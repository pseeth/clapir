...
abstract public class GraphView extends LinearLayout {
	...
	private class GraphViewContentView extends View {
		...
		protected void onDraw(Canvas canvas) {
			...
			for (int i = 1; i < verlabels.length-1; i++) {
	...
	protected void drawLegend(Canvas canvas, float height, float width) {
		int shapeSize = 15;
		int[] paints = new int[graphSeries.size()];
		int[] legendgraphs = new int[graphSeries.size()];
		...
		int numpaints = 0;
		boolean breakout;
		for (int j = 0; j < graphSeries.size(); j++) {
			breakout = false;
			for (int i = 0; i < paints.length; i++) {
				if (paints[i] == graphSeries.get(j).style.color) {
					breakout = true;
					break;
				}
			}
			if (!breakout) {
				paints[numpaints] = graphSeries.get(j).style.color;
		...
		float legendHeight = (shapeSize+5)*numpaints +5;
		...
		for (int i=0; i<numpaints; i++) {
			paint.setColor(graphSeries.get(legendgraphs[i]).style.color);
			...
			if (graphSeries.get(i).description != null) {
				...
				paint.setTextAlign(Align.LEFT);
				canvas.drawText(graphSeries.get(legendgraphs[i]).description, lLeft+5+shapeSize+5, lTop+shapeSize+(i*(shapeSize+5)), paint);
			}
		}
	}
